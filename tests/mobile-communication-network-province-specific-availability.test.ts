...
describe("mobile-communication-network-province-specific-availability Integration Tests", () => {
  ...
  describe("getMobileCommunicationNetworkProvinceSpecificAvailability", () => {
    it("should fetch availability data successfully", async () => {
      const response =
        await client.getMobileCommunicationNetworkProvinceSpecificAvailability();
  ...
  describe("getMobileCommunicationNetworkProvinceSpecificAvailabilityById", () => {
    let validId: number;
    ...
    beforeAll(async () => {
      ...
      if (response.value.length > 0) {
        ...
        console.warn(
          "No mobile communication network province-specific availability entries available for testing",
        );
      }
    });
    ...
    it("should fetch a single item by ID", async () => {
      if (validId) {
        const response =
          await client.getMobileCommunicationNetworkProvinceSpecificAvailabilityById(
            validId,
          );

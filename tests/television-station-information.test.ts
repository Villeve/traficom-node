...
describe("television-station-information Integration Tests", () => {
  ...
  describe("getTelevisionStationInformation", () => {
    it("should fetch a list of television station information", async () => {
      const response = await client.getTelevisionStationInformation({
        $top: 5,
      });
      ...
      if (response.value.length > 0) {
        ...
        expect(typeof item.ChannelNumber).toBe("number");
  ...
  describe("getTelevisionStationInformationById", () => {
    let validId: number;
    ...
    beforeAll(async () => {
      ...
      if (response.value.length > 0) {
        ...
        console.warn("No television station information available for testing");
      }
    });
    ...
    it("should fetch a single item by ID", async () => {
      if (validId) {
        const response =
          await client.getTelevisionStationInformationById(validId);
        ...
        expect(typeof response.ChannelNumber).toBe("number");
    ...
    it("should handle $select parameter correctly", async () => {
      if (validId) {
        const response = await client.getTelevisionStationInformationById(
          validId,
          {
            $select: "ID,Municipality,StationName",
          },

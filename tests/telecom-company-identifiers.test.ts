...
describe("telecom-company-identifiers Integration Tests", () => {
  ...
  describe("getTelecomCompanyIdentifiers", () => {
    it("should fetch a list of telecom company identifiers", async () => {
      const response = await client.getTelecomCompanyIdentifiers();
  ...
  describe("getTelecomCompanyIdentifierById", () => {
    let validId: number;
    ...
    beforeAll(async () => {
      ...
      if (response.value.length > 0) {
        ...
        console.warn("No telecom company identifiers available for testing");
      }
    });

    it("should fetch a single item by ID", async () => {
      if (validId) {
        const response = await client.getTelecomCompanyIdentifierById(validId);
    ...
    it("should throw TraficomError for non-existent ID", async () => {
      await expect(
        client.getTelecomCompanyIdentifierById(999999999),
